...
__global__ void rasterize_to_pixels_fwd_kernel(
    ...
    const float *__restrict__ uncertainties, // [C, N] or [nnz]
    ...
    float *__restrict__ render_uncertainties, // [C, image_height, image_width, 1]
    ...
    render_uncertainties += camera_id * image_height * image_width;
    ...
    float3 *xy_opacity_batch = (float3 *)&id_batch[block_size];    // [block_size]
    float3 *conic_batch = (float3 *)&xy_opacity_batch[block_size]; // [block_size]
    float *uncertainty_batch = (float *)&conic_batch[block_size];  // [block_size]
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
        ...
        if (idx < range_end) {
            ...
            xy_opacity_batch[tr] = {xy.x, xy.y, opac};
            conic_batch[tr] = conics[g];
            uncertainty_batch[tr] = uncertainties[g];
        }
    ...
    if (inside) {
        ...
        render_uncertainties[pix_id] = pix_uncertainty;
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> rasterize_to_pixels_fwd_tensor(
    ...
    const torch::Tensor &uncertainties // [C, N] or [nnz]
    ...
    case 1:
        ...
        rasterize_to_pixels_fwd_kernel<1><<<blocks, threads, shared_mem, stream>>>(
            ...
            opacities.data_ptr<float>(), uncertainties.data_ptr<float>(),
